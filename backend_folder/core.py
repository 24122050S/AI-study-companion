...
# 🔧 SỬA LỖI: "llama3-8b-8192" và "mixtral-8x7b-32768" đã bị Groq GỠ BỎ (decommissioned).
# Để chúng trong danh sách dự phòng khiến vòng lặp fallback luôn thất bại và tốn thời gian chờ.
FALLBACK_MODELS = [GROQ_MODEL, "llama-3.1-8b-instant"]
...
def get_active_context(query: str, user_id: str, notebook_id: str, k_needed: int = 15, return_refs: bool = False):
    ...
    try:
        ...
        for row in res.data:
            ...
            if return_refs:
                # 🔧 SỬA LỖI LOGIC BỊ NGƯỢC: hàm match_documents của Supabase trả về
                # "similarity" CÀNG CAO = CÀNG GIỐNG. Code cũ viết `score <= 0.55`
                # tức là GIỮ LẠI các đoạn KÉM liên quan và VỨT BỎ các đoạn TỐT NHẤT,
                # khiến AI chat trả lời lạc đề hoặc báo "không có trong tài liệu".
